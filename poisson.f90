...
implicit none
 ...
 integer omp_get_num_threads,nthreads,omp_get_thread_num
 real(KND),dimension(:),allocatable::Apx,Apy,Apz,Aw,Ae,As,An,Ab,At
...
!     write (*,*) "-",k,j
  ...
  if (called==0) then
   allocate(PH(1:nx,1:ny,1:nz))
   allocate(RH(1:nx,1:ny,1:nz))
  endif

  RH=0
  RH(1+sx:Prnx+sx,1+sy:Prny+sy,1+sz:Prnz+sz)=RHS(1:Prnx,1:Prny,1:Prnz)
  if (BtypeE==PERIODIC)   RH(nx,:,:)=RH(1,:,:)
  if (BtypeN==PERIODIC)   RH(:,ny,:)=RH(:,1,:)
  if (BtypeT==PERIODIC)   RH(:,:,nz)=RH(:,:,1)
  Ph=0
  ...
  if (called==0) then
   ...
   iparm(18)=maxPoissoniter !max cycles
   iparm(19)=0 !gaus seidel, no other option
   iparm(20)=0 !0 size of workspace for first call
   ...
   mgopt(1)=1 !V cycle
   mgopt(2)=2 !prerelaxation sweeps
   mgopt(3)=1 !postrelaxation sweeps
   mgopt(4)=1 !linear prolongation

   allocate (work(0))
   
   if (KND==DBL) then
    call dmud3sp(iparm,fparm,work,MUDcx,MUDcy,MUDcz,MUDbc,rh,ph,mgopt,ierror)
   else
    call mud3sp(iparm,fparm,work,MUDcx,MUDcy,MUDcz,MUDbc,rh,ph,mgopt,ierror)
   endif
   
   if (ierror/=0) then
    write (*,*) "call 1 ierror",ierror
   endif

   deallocate(work)
   allocate(work(iparm(21)))
   iparm(20)=iparm(21)
  ...
  Phi(1:Prnx,1:Prny,1:Prnz)=Ph(1+sx:Prnx+sx,1+sy:Prny+sy,1+sz:Prnz+sz)
   
  write (*,*) "MG cycles",iparm(22)
  write (*,*) "MG error",fparm(8)
   if (ierror/=0) then
    write (*,*) "ierror",ierror
   endif
