...
contains
  ...
  subroutine read_header(g, lo, up, staggered, periodic)
    use Endianness
    class(grid),intent(inout) :: g
    real(rp), intent(in) :: lo, up
    integer, intent(in) :: staggered
    logical, intent(in) :: periodic
    integer :: io, nx, ny, nz, j
    character(5) :: ch5
    character(70) :: str
    character :: ch
    ...
    if (any(abs((g%z(3:nz)-g%z(2:nz-1))/g%dz-1)>epsilon(1._rp))) then
      g%dz = nan_rp
      write (*,*) "Error, still requiring uniform grid input"
      stop
    end if
...
!       if (periodic) then
!         do j = -1, -2, -1
!           g%z(j) = lo - (up-g%z(nz+j))
!         end do
!         do j = nz+1, g%nz+3
!           g%z(j) = up + (g%z(j-nz)-lo)
!         end do
!       else
!         do j = -1, -2, -1
!           g%z(j) = lo - (g%z(0-j)-lo)
...
program interpolate_new_grid_variable_z
  use WorkKinds
  use Types
  use Endianness
  use Interpolation
  ...
  character(:), allocatable :: file_name, base_name, dir_name, title, bc_dir, grid_file
  ...
  if (command_argument_count()<2) then
    write(*,*) "Usage: interpolate_new_grid file_name [nx,ny,nz]/[nx,ny,grid_file]"
    write(*,*) "nx,ny,nz are the numbers of cells in each dimension and grid_file is the vertical grid file_name."
    stop 1
  end if
  ...
  allocate(character(1024) :: grid_file)
  
  call get_command_argument(2, value=arg)
  read(arg,*,iostat=io) new%nx, new%ny

  grid_file = trim(arg(index(arg,",",.true.)+1:))
  ...
  call read_grid_file(grid_file)

  old%fname = file_name

  call old%read_header(lo, up, staggered, enable_bc.and.Btype(To)==BC_PERIODIC)
...
contains
  ...
  subroutine read_grid_file(fname)
    character(*), intent(in) :: fname
    integer :: u
    integer :: j, nz
    real(rp), allocatable :: zu(:)
    real(rp) :: p
    ...
    if (staggered==3) then
      ...
      if (Btype(To)==BC_PERIODIC) then
        do j = -1, -2, -1
          new%z(j) = lo - (up-new%z(nz+j))
        end do
        do j = nz+1, new%nz+3
          new%z(j) = up + (new%z(j-nz)-lo)
        end do
      else
        do j = -1, -2, -1
          new%z(j) = lo - (new%z(0-j)-lo)
        end do
        do j = nz+1, new%nz+3
          new%z(j) = up + (up-new%z(nz-(j-nz)))
        end do
      end if     
      new%z1 = new%z(1)
    else  
      ...
      new%nz = nz
      allocate(new%z(-2: new%nz+3))
      new%z(1:nz) = [((zu(j)+zu(j-1))/2, j=1,nz)]
      if (Btype(To)==BC_PERIODIC) then
        do j = 0, -2, -1
          new%z(j) = lo - (up-new%z(nz+j))
        end do
  ...
  function interpolate_trilinear(arr, x, y, z) result(res)
    ...
    res = TriLinInt((x - old%x(xi)) / (old%x(xi+1)-old%x(xi)), &
                    (y - old%y(yj)) / (old%y(yj+1)-old%y(yj)), &
                    (z - old%z(zk)) / (old%z(zk+1)-old%z(zk)), &
                    arr(xi, yj, zk), &
                    arr(xi+1, yj  , zk  ), &
                    arr(xi  , yj+1, zk  ), &
                    arr(xi  , yj  , zk+1), &
