...
implicit none
 ...
 type WMpoint   !points in which apply wall model
  integer x
  integer y
  integer z
  real(KND) distx
 ...
 contains
  ...
  if (wallmodeltype==1) then
   ...
   kprime=0.41*(1-dptrans*(1./(0.41*ustar0**2)-1./(2*ustar0**2)))
   yplcrit=-LambertW(-kprime*0.119)/kprime
   if ((dist*ustar0*Re)<yplcrit) then
  ...
  type(WMPoint):: WMP 
   ...
   !abs(ProjectU(n,U)) !nepresne, je treba udelat projekci celeho vektoru do roviny rovnobezne se stenou
   ustar=WMP%ustar
   ustar=WM1ustar(vel,dist,ustar,dp,dptrans)
   !write (*,*) "!!"
   WMP%ustar=ustar
10 if (ustar<0) ustar=0
   if (vel>0) then 
  ...
  integer i
!   write (*,*) vel,dist,z0,ustar,tempflux,tempdif
...
10   continue
  ...
  type(WMPoint):: WMP 
   ...
   if (ustar<0) ustar=0
   WMP%ustar=ustar
20 if (vel>0.and.ustar*ustar*dist/vel>1._KND/Re) then
